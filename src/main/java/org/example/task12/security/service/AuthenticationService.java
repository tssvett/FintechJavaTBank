...
public class AuthenticationService {
    ...
    public AuthenticationResponse register(RegistrationRequest request) {
        ...
        ApiUser apiUser = ApiUser.builder().login(request.username()).password(
                passwordEncoder.encode(request.password())).role(Role.USER).build();
        ...
        jwtRepository.save(JwtToken.builder().token(jwtToken).revoked(false).user(apiUser).build());
    ...
    public void logout(String jwtToken) {
        jwtToken = jwtToken.replace(JwtAuthenticationFilter.BEARER, "").trim();
        JwtToken token = jwtRepository.findByToken(jwtToken)
